...
\begin{itemize}
        ...
        \\
         The most advanced models in biology that we can see today are inspired by tools that were originally built for understanding human language, specifically attention mechanisms and transformer architecture. These models have the ability to find complex patterns and relationships in various types of data at the same time, and because of this they were considered very powerful models.\cite{wu2022deep}.
    
        For example, a model called DeepCE uses a special attention technique to figure out how the tiny building blocks of a drug interact with specific genes. When looking at untested, new chemicals and how they might affect gene activity, which is a huge step forward for drug discovery, this model helps in the prediction\cite{pham2021deep}.
    
        Another model, MultiDCP, goes more deep in this path. This model uses biological knowledge alongside transformer architecture to predict how cells will respond to drugs, when there are completely new cell types available. Remarkably, it can sometimes make predictions that are more reliable than actual experimental results, which are often noisy or inconsistent\cite{wu2022deep}.

        When it comes to accuracy and scale, these models are pushing to the maximum limit, but with this higher accuracy and scale, there is a tradeoff as well. The workings inside the models become highly complex, which makes it difficult for the researchers to explain exactly why this prediction happened and how they can validate that prediction. Although this trade-off is there, these models represent a huge leap in computational biology.\cite{chow2022predicting}.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Sources of Noise}
    \begin{itemize}
            ...
            One of the main reasons that the L1000 dataset has a lot of errors is its technical process. The method they used to separate signal peaks was originally a k-means algorithm, and it turned out to be very unreliable, which led to inaccurate gene expression readings. To fix this issue, researchers have come up with better approaches, like Gaussian Mixture Models (GMM), Aggregate GMM (AGMM), and more recently, Bayesian models that can be considered more stable when compared to others, and these models can also deal with things like outliers and uncertainty better than other models\cite{qiu2020bayesian}.
        
            Another big challenge is that the L1000 system only measures 978 landmark genes directly. The rest of the genes, which are about 11,000, are actually not measured but are generated using algorithms. This actually means that the rest of the dataset is, which is a large part considering the 978 and the 11,000, based on predictions, which can be less reliable, and adds another layer of noise\cite{subramanian2017next,mcdermott2019deep}.
            \item \textbf{Metadata Inconsistency and Label Noise: }
            Another huge inconsistency in the LINCS L1000 dataset is the annotations associated with the data. They show huge errors and inconsistencies.  This includes missing MoA labels, inconsistent drug identifiers, and heterogeneity in how information is recorded. When using supervised models, these issues translate directly into label noise that can really damage the model more than the noisy features\cite{szalai2019signatures}. 
            \end{itemize}
        \item \textbf{Mitigation Strategies:}
    
            \begin{itemize}
            \item \textbf{Data-Centric Approaches:}
            The methods that are currently being used are designed to improve data quality before starting the modelling stage. When it comes to these kinds of situations, one of the best practices is to use superior signature generation methods like the Characteristic Direction (CD), which has been proven and shown to improve the signal-to-noise ratio over default methods, significantly\cite{duan2016l1000cds2}.Also, data cleaning on replicate correlation is used as a common technique to filter out unreliable experiments\cite{szalai2019signatures}. If a more advanced strategy is needed, then the data augmentation strategy in DeepCE can be mentioned. It uses a trained model to identify and salvage reliable bio-replicated data from experiments initially flagged as noisy, thereby increasing the size of the high-quality training se\cite{pham2021deep}.
                \\
            \item \textbf{Model-Centric Approaches:} 
            The primary approach is to come up with models that are robust to noise. It is an excellent field of research and multiple methods are used.
            \begin{itemize}
            \item \textbf{Robust Loss Functions:} To keep models from overfitting to label noise, we can use loss functions like Mean Absolute Error (MAE) or generalised cross-entropy, that are bounded and not as sensitive to easily inaccurate (e.g., mislabeled) examples\cite{szalai2019signatures}.
            \item \textbf{Robust Training Procedures:}To avoid the memorisation of noisy labels, methods like co-teaching are sometimes used, where two networks are trained in parallel and teach one another using only their most reliable (lowest-loss) predictions\cite{szalai2019signatures}. Curriculum training is another effective strategy where a model is first trained on simple/clean examples before being exposed to more difficult/noisy data\cite{pham2021deep}.
            \item \textbf{Self-Supervised and Semi-Supervised Learning:} These paradigms are very helpful for leveraging large amounts of unlabelled or noisy labelled data.  Models can be  trained earlier on a task that does not require clean labels (for example, a denoising autoencoder that learns to reconstruct a clean profile from a corrupted one) in order to learn robust representations of the data structure. The model that has been trained earlier, can be fine-tuned on a smaller, cleaner dataset for the final prediction task.\cite{bang2024transfer}.
    \end{itemize}
  \end{itemize}
\end{itemize}
\textbf{R3: Integration of External Knowledge and Impact on Interpretability}
The most advanced frameworks realize that gene expression is only one part of the problem, and the model integrates external knowledge all the time to create models with more power.
\\
\begin{itemize}
    \item \textbf{Rationale and Fusion Architectures:} The main goal is to combine complementary information. For example, a drug's chemical structure decides the physical properties and target areas of that drug, while biological networks (e.g., PPIs, Gene Ontology)  provide the details or the context of the cells in which the drug acts\cite{liu2015compound}. Flexible architectures are provided by deep learning for multimodal fusion. While simple early fusion (combining raw features) is possible, intermediate fusion is the more dominant strategy. This involves using a lot of separate branches of networks before fusing them in a deep layer in order to learn high-level representations of each modality\cite{stahlschmidt2022multimodal}. Recent models use complex fusion mechanisms like cross-attention, which allows the model to learn the alignments between the modalities whenever they change, like which part of the drug's structure is matching and most relevant to the gene pathways.\cite{elabd2024simple}.
